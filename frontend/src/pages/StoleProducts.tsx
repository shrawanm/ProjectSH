...
export function StoleProducts() {
  ...
  const allProducts = getProductsByCategory('pashmina', 'stole');
  const filteredProducts = selectedBlend === 'all' ? allProducts : getProductsByCategory('pashmina', 'stole', selectedBlend);
  if (!subcategory) {
  ...
  return <div className="min-h-screen pt-24 pb-12 bg-bg-light dark:bg-bg-dark">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <div className="flex items-center gap-2 text-sm text-text-secondary">
          ...
          <Link to="/pashmina" className="hover:text-accent transition-colors">
            Pashmina
          </Link>
          <ChevronRight className="w-4 h-4" />
      ...
      {subcategory.children && <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
          <div className="flex items-center gap-3 justify-center flex-wrap">
            ...
            <button onClick={() => setSelectedBlend('all')} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedBlend === 'all' ? 'bg-accent text-white' : 'bg-gray-100 dark:bg-gray-800 text-text-secondary hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
              All Stoles
            </button>
            {subcategory.children.map(child => <button key={child.id} onClick={() => setSelectedBlend(child.id)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedBlend === child.id ? 'bg-accent text-white' : 'bg-gray-100 dark:bg-gray-800 text-text-secondary hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
                {child.name}
              </button>)}
          </div>
        </div>}
