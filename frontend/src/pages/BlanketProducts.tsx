...
export function BlanketProducts() {
  ...
  const subcategory = category?.subcategories?.find(s => s.id === 'blanket');
  const products = getProductsByCategory(mainCategory, 'blanket');
  if (!subcategory) {
  ...
  return <div className="min-h-screen pt-24 pb-12 bg-bg-light dark:bg-bg-dark">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {products.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <Link to={`/${mainCategory}`} className="mt-4 inline-block text-accent hover:text-accent-hover font-medium">
