...
export function MufflerProducts() {
  ...
  const products = getProductsByCategory('pashmina', 'muffler');
  if (!subcategory) {
  ...
  return <div className="min-h-screen pt-24 pb-12 bg-bg-light dark:bg-bg-dark">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {products.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <Link to="/pashmina" className="mt-4 inline-block text-accent hover:text-accent-hover font-medium">
              Browse other Pashmina products
            </Link>
          </div>}
      </div>
    </div>;
